...
const EditResume = () => {
  ...
  // Update  the array Items state using Index
  ...
  // Adding new items IE Information 
  ...
  //removieng the Info using index 
  ...
  const fetchResumeDetailsById = async () => {
    try {
      const response = await axioInstance.get(API_PATHS.RESUME.GET_BY_ID(resumeId))
  ...
  // It will help in Choosing the preview as  well as helps in downloading the resume also save the resume as Images
   const uploadResumeImages = async () => {
    try {
      ...
      const uploadResponse = await axioInstance.put(
  ...
  const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      await axioInstance.put(API_PATHS.RESUME.UPDATE(resumeId), {
        ...resumeData,
...
// Dwonlad Funcion 
  const downloadPDF = async () => {
    ...
    // Type of how it wil look when downloaded 
    try {
      await html2pdf()
        .set({
          margin:       0,
        ...
        .from(element)
        .save(); // we will save the resume here and then we download it ...
  ...
  const handleDeleteResume = async () => {
    try {
      setIsLoading(true)
      await axioInstance.delete(API_PATHS.RESUME.DELETE(resumeId))
